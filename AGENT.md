...
- **No runtime build system, no bundler, no npm dependencies** for the app itself.
  `build-single-file.js` is only an optional zero-dependency generator for the single-file distribution.
...
├── ai-chinese-chess.html   Generated single-file build (do NOT hand-edit; run node build-single-file.js)
...
- **Add a UI feature**: edit `index.html` for markup, `css/style.css` for style,
  ...
  when feasible. Then regenerate the single-file build (`node build-single-file.js`).
- **Add a JS module**: create `js/xxx.js` as an IIFE attaching to `globalThis`,
  ...
  Then regenerate the single-file build (`node build-single-file.js`).
...
- Do not hand-edit `ai-chinese-chess.html`; always regenerate via `node build-single-file.js`.
